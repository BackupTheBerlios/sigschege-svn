...
void EasyVecElmCompound::get_limits(EVPosInt &upper_left, EVPosInt &lower_right) {
  ...
  if (members_iter==members.end()) {
    ...
    (*members_iter)->get_limits(member_point_min, member_point_max);
...
vector<EasyVecElm*> EasyVecElmCompound::flat_list() {
  ...
  for ( members_iter = members.begin(); members_iter != members.end(); ++members_iter ) {
    tmp_list = (*members_iter)->flat_list();
...
void EasyVecElmCompound::save_elm(ofstream &fig_file) {
  EVPosInt pos_min, pos_max;
  get_limits(pos_min, pos_max);
...
void EasyVecElmCompound::save_content(ofstream &fig_file) {
  vector<EasyVecElm*>::iterator members_iter;
  for ( members_iter = members.begin(); members_iter != members.end(); ++members_iter ) {
    (*members_iter)->save_elm(fig_file);
  }  
}
