...
Handle<Event> EventList::createEvent(const std::string &eventNewState,
		double eventDelay, Handle<Event> *refEvent_p) {
...
Handle<Event> EventList::getEventAfter(double evTime, int percentageLevel,
	...
	for (eventsIter = events.begin(); eventsIter != events.end(); ++eventsIter) {
		if (eventsIter->Object()->getTime(percentageLevel) >= evTime && (newState
				== string("") || newState == eventsIter->Object()->getNewState())) {
			return *eventsIter;
		}
	}
